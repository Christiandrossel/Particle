...
public class _04_BlurEffectShader {
	...
	public static void main(String[] args) throws LWJGLException {
		try {						
			...
			// **********************************************************************************************
			// Shader für den Blur-Effekt
			String fragShaderBlureffect = 
			...
			glShaderSource(shaderObjectF, fragShaderBlureffect);
			glCompileShader(shaderObjectF);

			// Programm erzeugen, mit Shader verknüpfen und das Programm linken
			int programBlureffect = glCreateProgram();
			glAttachShader(programBlureffect, shaderObjectF);
			glLinkProgram(programBlureffect);	

			// Parameter, die für den Shader relevant sind, als Uniforms 
			// mit dem Programm verknüpfen
			int sLoc 	= glGetUniformLocation(programBlureffect, "s");
			int tex1Loc = glGetUniformLocation(programBlureffect, "tex1");
			int tex2Loc = glGetUniformLocation(programBlureffect, "tex2");
			...
			String fragShaderBlureffectVisualisation = 
				"uniform sampler2D tex1;"+
				"uniform sampler2D tex2;" +
				"" +
				"uniform vec2 s;"+
				"uniform float scale;"+
				"void main (void) {"+	
				"	gl_FragColor = texture2D(tex1, gl_TexCoord[0].st) * vec4(1f, 1f, 0f, 1f) * 5.0f;" +
				"}";			
			
			int shaderObjectF2 = glCreateShader(GL_FRAGMENT_SHADER);
			glShaderSource(shaderObjectF2, fragShaderBlureffectVisualisation);
			glCompileShader(shaderObjectF2);

			int programBlureffectVisualisation = glCreateProgram();
			glAttachShader(programBlureffectVisualisation, shaderObjectF2);
			glLinkProgram(programBlureffectVisualisation);			
			
			int sLoc2 		= glGetUniformLocation(programBlureffectVisualisation, "s");
			int scaleLoc2 	= glGetUniformLocation(programBlureffectVisualisation, "scale");
			int tex1Loc2 	= glGetUniformLocation(programBlureffectVisualisation, "tex1");
			int tex2Loc2 	= glGetUniformLocation(programBlureffectVisualisation, "tex2");
						
			glUseProgram(programBlureffectVisualisation);		
			
			glUniform2f(sLoc2, 1.0f/WB, 1.0f/HB);			
			glUniform1f(scaleLoc2, -200.0f);
			...
			errorBuffer.rewind();
			glGetProgram(programBlureffectVisualisation, GL_LINK_STATUS, errorBuffer);
			System.out.println(errorBuffer.get(0)==GL_TRUE?"OK":"ERROR");
			
			int error = errorBuffer.get(0);
			errorBuffer.put(0,1024);
			glGetProgramInfoLog(programBlureffectVisualisation, errorBuffer, infoBuffer);		
			if (error!=GL_TRUE) 
			{
